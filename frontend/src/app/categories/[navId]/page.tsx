...
type Category = { id: string; name?: string; title?: string; slug?: string | null };
...
export default async function Page({ params }: { params: Promise<{ navId: string }> }) {
    ...
    <main className="container-xl py-8 space-y-6">
      ...
      <ul className="divide-y divide-white/10 rounded-xl border border-white/10">
        {cats.map((c) => {
          ...
          const slug = c.slug ?? title.toLowerCase().replace(/\s+/g, '-');
          return (
            <li key={c.id} className="flex items-center justify-between p-4">
              <div>{title}</div>
              <Link href={`/products/${slug}?page=1&limit=12`} className="btn">Browse</Link>
            </li>
          );
        })}
      </ul>
